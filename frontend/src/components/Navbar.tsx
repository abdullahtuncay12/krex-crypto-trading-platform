...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="relative z-50 bg-gray-50 dark:bg-crypto-dark-800/80 dark:backdrop-blur-md border-b border-gray-200 dark:border-crypto-dark-500/50 transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
            ...
            {isAuthenticated ? (
              <>
                ...
                {user?.role === 'normal' && (
                  <Link
                    to="/premium"
                    className="px-3 py-2 text-sm font-medium bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white rounded transition font-semibold whitespace-nowrap flex items-center space-x-1"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <span>{language === 'tr' ? 'Premium' : language === 'ru' ? 'Премиум' : language === 'ja' ? 'プレミアム' : language === 'de' ? 'Premium' : 'Premium'}</span>
                  </Link>
                )}
                <Link
                  to="/bot/create"
                ...
                <div className="flex items-center space-x-2 px-2 py-1.5 bg-green-50 dark:bg-crypto-dark-700 border border-buy/30 rounded whitespace-nowrap">
                ...
                <div className="flex items-center space-x-2 px-2 py-1.5 bg-gray-100 dark:bg-crypto-dark-700 rounded whitespace-nowrap">
                ...
                <button
                  onClick={toggleTheme}
                  className="px-2 py-1.5 text-xs font-medium text-gray-300 dark:text-gray-300 hover:text-white hover:bg-gray-200 dark:hover:bg-crypto-dark-600 rounded transition whitespace-nowrap border border-gray-300 dark:border-crypto-dark-500"
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-md text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-crypto-dark-600 focus:outline-none"
          >
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-gray-200 dark:border-crypto-dark-500">
            ...
            {isAuthenticated ? (
              <>
                <Link
                  ...
                  className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-crypto-dark-600"
                >
                  {t('nav.home')}
                </Link>
                <Link
                ...
                {user?.role === 'normal' && (
                  <Link
                    to="/premium"
                    onClick={closeMobileMenu}
                    className="block px-3 py-2 rounded-md text-base font-medium bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white"
                  >
                    ⭐ {language === 'tr' ? 'Premium\'a Yükselt' : language === 'ru' ? 'Обновить до Premium' : language === 'ja' ? 'プレミアムにアップグレード' : language === 'de' ? 'Auf Premium upgraden' : 'Upgrade to Premium'}
                  </Link>
                )}
                <Link
                  to="/bot/create"
                ...
                <button
                  onClick={toggleTheme}
                  className="w-full text-left px-3 py-2 rounded-md text-base font-medium text-gray-300 dark:text-gray-300 hover:text-white hover:bg-gray-200 dark:hover:bg-crypto-dark-600 border border-gray-300 dark:border-crypto-dark-500"
                >
